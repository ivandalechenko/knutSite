...
const ThimblesGame = (props) => {
    ...
    const play = async (num) => {
        if (canPlay) {
            const res = await api.post('/game/play', { gameId: gameStore.thimblesStatus })
            gameStore.thimblePlay()
            const win = res.data.status === 'win'
...
export default observer(ThimblesGame)
